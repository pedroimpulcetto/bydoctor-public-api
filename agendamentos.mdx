...
|---|---|---|---|
| `patient_id` | `integer` | sim | `patient.id` de um paciente **ativo** da clínica. |
| `professional_id` | `integer` | sim | `professional.id` de um profissional ativo da clínica que atende pacientes. |
...
| `room_id` | `integer` | não | `room.id` de uma sala ativa. Omitido, a API usa a sala padrão da grade do profissional, se houver. |
...
- no tipo "Consulta" da clínica e com o pagador Particular do profissional — tipo, convênio e valor são ajustados no aplicativo, se necessário.

Os ids de paciente, profissional e sala só resolvem dentro da clínica da chave. Um id de outra clínica e um id inexistente recebem **a mesma** resposta `400`, no formato de erro por campo:
...
Envie o cabeçalho `Idempotency-Key` (até 255 caracteres, um UUID serve) para poder repetir a criação com segurança depois de um timeout ou erro de rede. Repetir a mesma requisição com a mesma chave em até **24 horas** devolve o agendamento já criado — mesmo `201`, mesmo corpo — com o cabeçalho de resposta `Idempotent-Replayed: true`, em vez de criar outro. A mesma chave com um corpo diferente responde `422` com `code: "idempotency_key_reused"`.
...
|---|---|
| `400 Bad Request` | Filtro ou corpo inválido. Nas escritas, o corpo traz o erro por campo: `{"campo": ["mensagem"]}`. Inclui id de paciente, profissional ou sala inexistente na sua clínica, `start_at` fora das regras, `status` fora de `scheduled`/`confirmed` e corpo vazio no `PATCH`. |
...
| `422 Unprocessable Entity` | `Idempotency-Key` reutilizada com um corpo diferente (`code: "idempotency_key_reused"`). |
...
Os objetos aninhados usam identificadores próprios: `patient.id`, `professional.id` e `room.id` são numéricos, os mesmos valores aceitos pelos filtros `patient_id`, `professional_id` e `room_id` e pelos campos de mesmo nome nas escritas. Se você pretende filtrar por eles ou criar agendamentos depois, guarde esses valores também.
