...
<Steps>
  <Step title="Crie uma chave de API">
    No ByDoctor, acesse **Minha Clínica → API e Webhooks** e crie uma nova chave. Toda chave nasce com o escopo de leitura (`appointments:read`); marque também **Escrever agendamentos** (`appointments:write`) se a integração vai criar, alterar ou cancelar agendamentos — os escopos não podem ser alterados depois. Veja [Escopos](/autenticacao#escopos).
