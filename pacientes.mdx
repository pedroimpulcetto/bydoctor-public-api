...
<Steps>
  ...
  <Step title="Agende com o id">
    Envie o `id` devolvido como `patient_id` em [`POST /appointments`](/agendamentos#criar-um-agendamento).
  </Step>
</Steps>
...
| `active` | `boolean` | `true` retorna só ativos, `false` só inativos. **Omitido, a lista inclui os dois.** |
...
| `gender` | `string` | sim | `F` (feminino), `M` (masculino) ou `O` (outro). O aplicativo também exige este campo. |
| `phone` | `string` | não | Celular com DDD, em qualquer formatação. Gravado em E.164. |
| `email` | `string` | não | E-mail válido. |
| `cpf` | `string` | não | Com ou sem pontuação. Validado pelos dígitos verificadores. |
| `birth_date` | `date` | não | `AAAA-MM-DD`. Não pode estar no futuro. |
...
|---|---|
| `400 Bad Request` | Filtro ou corpo inválido, com o erro por campo, por exemplo `{"cpf": ["Invalid CPF."]}`. Inclui campo obrigatório ausente, CPF, telefone ou e-mail inválido e corpo vazio no `PATCH`. |
...
| `404 Not Found` | `id` inexistente ou de outra clínica. |
